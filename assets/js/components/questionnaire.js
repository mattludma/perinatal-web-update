let questionnaireEPDS = document.querySelectorAll(
  '.questionnaire--questions-1'
);
let questionnaireEPDSIntro = document.querySelector(
  '#test-epds .questionnaire__intro'
);
let questionnaireEPDSQuestions = document.querySelector(
  '#test-epds .questionnaire__questions'
);
let questionnaireEPDSEvaluation = document.querySelector(
  '#test-epds .questionnaire__evaluation'
);

let questionnairePASS = document.querySelectorAll(
  '.questionnaire--questions-2'
);
let questionnairePASSIntro = document.querySelector(
  '#test-pass .questionnaire__intro'
);
let questionnairePASSQuestions = document.querySelector(
  '#test-pass .questionnaire__questions'
);
let questionnairePASSEvaluation = document.querySelector(
  '#test-pass .questionnaire__evaluation'
);
...
function nextQuestionnaire(questions) {
  ...
  if (questions === 'epds') {
    for (let radioInput of questionnaireEPDS[
      questionnaireQuestion
    ].querySelectorAll('input[type=radio]')) {
      if (radioInput.checked) {
        document.querySelector('#test-epds .is-open .is-error').style.opacity =
          '0';
        questionnaireQuestion = parseInt(questionnaireQuestion) + 1;
        if (
          parseInt(questionnaireEPDSQuestions.getAttribute('question')) <
          questionnaireEPDS.length - 1
        ) {
          questionnaireEPDSQuestions.setAttribute(
            'question',
            questionnaireQuestion.toString()
          );
          questionnaireEPDS[questionnaireQuestion - 1].classList.remove(
            'is-open'
          );
    ...
    for (let radioInput of questionnairePASS[
      questionnaireQuestion
    ].querySelectorAll('input[type=radio]')) {
      if (radioInput.checked) {
        document.querySelector('#test-pass .is-open .is-error').style.opacity =
          '0';
        questionnaireQuestion = parseInt(questionnaireQuestion) + 1;
        if (
          parseInt(questionnairePASSQuestions.getAttribute('question')) <
          questionnairePASS.length - 1
        ) {
          questionnairePASSQuestions.setAttribute(
            'question',
            questionnaireQuestion.toString()
          );
          questionnairePASS[questionnaireQuestion - 1].classList.remove(
            'is-open'
          );
